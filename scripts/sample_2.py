...
def generate_and_save_images(
    batch_size, total_samples, condition_value, path_out, file_prefix, a
):
    for i in range(0, total_samples, batch_size):
        ...
        for j, image in enumerate(results):
            file_name = f"{a}/{file_prefix}_{i + j}.jpg"
...
if __name__ == "__main__":
    ...
    for index in range(5):
        ...
        cond = torch.tensor(index).to(device)
        cond_path = path_out / f"{index}"
        cond_path.mkdir(parents=True, exist_ok=True)
        generate_and_save_images(batch_size, n_samples, cond, path_out, "test", f"{index}")
