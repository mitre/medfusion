...
def find_most_similar_pairs(directory1, directory2, output_directory, top_n=10):
    ...
    """
    images1 = [(path, load_and_resize_image(path)) for path in tqdm(list(Path(directory1).rglob('*.[jp][pn]g')), desc="Loading images from directory 1")]
    images2 = [(path, load_and_resize_image(path)) for path in tqdm(list(Path(directory2).rglob('*.[jp][pn]g')), desc="Loading images from directory 2")]
...
# Example usage
directory1 = '/projects/NEI/pranay/Eyes/Datasets/Diff_Generated_experiment_fundus_1024_cond'
